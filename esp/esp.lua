...
local function GetBoxFromAnchor(anchor, model, distance)
    ...
    if distance > 600 then
        local HEIGHT = anchor.Size.Y * 2.5
        local WIDTH = math.max(anchor.Size.X, anchor.Size.Z) * 1.5
...
function SkeletonSystem:RenderSkeleton(model, lines, color, simplify)
    ...
    for _, bone in ipairs(connections) do
        ...
        if part1 and part2 and line and part1:IsDescendantOf(workspace) and part2:IsDescendantOf(workspace) then
            local success, pos1, vis1, pos2, vis2 = pcall(function()
                local p1, v1 = Camera:WorldToViewportPoint(part1.CFrame.Position)
                local p2, v2 = Camera:WorldToViewportPoint(part2.CFrame.Position)
                return p1, v1, p2, v2
            end)
            
            if success and vis1 and vis2 then
...
function SkeletonSystem:UpdateLocalPlayerSkeleton()
    ...
    if now - self.LocalPlayerSkeleton.LastUpdate < 0.05 then
        return
    end
    ...
    for key, obj in pairs(storage) do
        if key:sub(1, 1) ~= "_" and key ~= "SkeletonLines" and key ~= "Highlight" then
            if obj then
                ...
                elseif key:find("Health") then objType = "Square"
                end
                
                if objType then
                    DrawingPool:Release(objType, obj)
                end
            end
        end
    end
    
    for _, line in pairs(storage.SkeletonLines) do
...
local function HideESP(storage)
    if storage.Box then 
        ...
        storage.Box.Transparency = 0
    end
    if storage.BoxOutline then 
        storage.BoxOutline.Visible = false
        storage.BoxOutline.Transparency = 0
    end
    ...
    if storage.Highlight and storage.Highlight.Enabled then
...
function ESP:UpdatePlayerESP(player, storage)
    ...
    if PerformanceManager.panicLevel >= 2 then
        ...
        if visible and w > 0 and h > 0 then
    ...
    -- Skeleton
    local skeletonMaxDist = Settings.Drawing.SkeletonMaxDistance
    ...
    if storage.Highlight and Settings.ShowHighlight and distance < 500 then
        ...
        if not storage.Highlight.Enabled then
